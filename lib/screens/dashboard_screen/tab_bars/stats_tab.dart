...
class _StatsTabState extends State<StatsTab> {
  ...
  void getData() async {
    await Provider.of<LandProvider>(context,listen: false).getLands().then((onValue) async {
      ...
      await Provider.of<WeatherProvider>(context,listen: false).getRainForecastedData(landProvider.lands!.first.lat, landProvider.lands!.first.lng);
      ...
      await Provider.of<HealthScoreprovider>(context,listen: false).getLastScanDate(landProvider.lands!.first.id);
    }).catchError((onError){
      print(onError);
    });
  }
  ...
  Widget build(BuildContext context) {
    return Consumer2<WeatherProvider,HealthScoreprovider>(
      builder: (BuildContext context, WeatherProvider weather_provider,HealthScoreprovider health_provider, Widget? child) {
        return SingleChildScrollView(
          child: Column(
            children: [
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Align(
                              ...
                              child: Text("HUMIDITY ${weather_provider.area_humidity}%",style: TextStyle(fontSize: 14,color: Colors.grey,fontWeight: FontWeight.w700),)),
                          Align(
                              alignment: Alignment.topLeft,
              ...
              SizedBox(height: 5,),
             (health_provider.lastScanDate??'').isNotEmpty?Text("Last Scan : ${DateFormat("dd MMM yyy").format(DateTime.parse(health_provider.lastScanDate??''))}",style: TextStyle(fontSize: 14,color: Colors.grey),):SizedBox()
            ],
          ),
        );
      },
